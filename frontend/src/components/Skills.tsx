...
export default function Skills() {
  const categories = ["Frontend", "Backend", "Security", "Other"];
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-white/10 border border-white/10">
          {categories.map((cat, catIdx) => (
            <div key={cat} className="bg-ink p-8 md:p-10 flex flex-col gap-10">
              <div className="flex items-center gap-6">
                <span className="text-4xl md:text-5xl font-black text-accent/20 italic leading-none">0{catIdx + 1}</span>
                <h3 className="text-lg md:text-xl font-bold uppercase tracking-widest text-paper/80">{cat}</h3>
              </div>
              
              <div className="flex flex-col gap-8">
                {SKILLS.filter(s => (cat === "Security" ? s.category === "Design" : s.category === cat)).map((skill, idx) => (
